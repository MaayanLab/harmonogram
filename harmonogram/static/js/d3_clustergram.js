...
function initialize_visualization(network_data, params){
  ...
  var ini_clust_height = params.svg_dim.height - (params.super_label_width + 0.8*label_scale(col_max_char) + params.class_room.col) - 5*params.grey_border_width ;
  ...
  var scale_reduce_font_size_factor = d3.scale.log().domain([min_node_num,max_node_num]).range([min_fs_zoom,max_fs_zoom]).clamp('true');
  ...
  var scale_fs_screen_width = d3.scale.linear().domain( [min_viz_width,max_viz_width]).range([0.75,1.15]).clamp('true');
  ...
  var real_zoom_scale_col = d3.scale.linear().domain([min_node_num,max_node_num]).range([2,5]).clamp('true');
  ...
  var  real_zoom_scale_screen = d3.scale.linear().domain([min_viz_width,max_viz_width]).range([2,1]).clamp('true');
...
function make_d3_clustergram(args) {
  ...
  else if ( args.order == 'clust' || args.order == 'rank' || args.order == 'class' ) {
  ...
  var container_all_row = d3.select('#main_svg')
    .append("g")
    .attr("transform", "translate(" + params.norm_label.margin.left + "," + params.clust.margin.top + ")");
  ...
  if(_.has(params,'group_colors')){
    ...
    row_class_rect = row_triangle_ini_group
      .append('rect')
      ...
      if (typeof params.click_group == 'function'){
        ...
        row_class_rect
          .on('click',function(d,i){
            ...
            group_info.info = {'type':'distance','cutoff':inst_level/10};
  ...
  var container_all_col = d3.select('#main_svg')
    .append("g")
    .attr("transform", "translate(" + params.clust.margin.left + "," + params.norm_label.margin.top + ")");
...
function parent_div_size_pos( params ){
  ...
  if (params.resize == true){
    ...
    var container_dim = {};
    container_dim.width  = screen_width  - outer_margins.left - outer_margins.right;
    container_dim.height = screen_height - outer_margins.top - outer_margins.bottom;
...
function apply_transformation( trans_x, trans_y, zoom_x, zoom_y ){
  ...
  params.clust_group
    .attr('transform','translate(' + [ trans_x, trans_y ] + ') scale('+ zoom_x +',' + zoom_y + ')');
  ...
  d3.select('#row_label_triangles')
    .attr('transform','translate(' + [ 0 , trans_y ] + ') scale( 1,' + zoom_y + ')');
...
function two_translate_zoom( pan_dx, pan_dy, fin_zoom){
  ...
  if (d3_clustergram.params.run_trans == 0){
    ...
    if (pan_dy >= half_height - y_pan_room){
      ...
      var shift_up_viz  = - half_height/params.zoom_switch + shift_top_viz ; 
    ...
    params.clust_group
      ...
      .attr('transform', 'translate(' + [ 0,  0 + center_y ] + ')'+' scale('+ 1 +',' + zoom_y + ')'+'translate(' + [  pan_dx,  pan_dy ] + ')');

    // transform row labels 
    d3.select('#row_labels')
      .transition()
      .duration(search_duration)
      .attr('transform', 'translate(' + [ 0, center_y ] + ')'+' scale('+ zoom_y +',' + zoom_y + ')'+'translate(' + [  0,  pan_dy ] + ')');
    ...
    d3.select('#row_label_triangles')
      .transition()
      .duration(search_duration)
      .attr('transform', 'translate(' + [ 0, center_y ] + ')'+' scale('+ 1 +',' + zoom_y + ')'+'translate(' + [  0,  pan_dy ] + ')');
    ...
    var net_y_offset = params.clust.margin.top + center_y +  pan_dy * zoom_y  ;
...
function reorder_click_row(d,i){
  ...
  for (j=0; j<col_nodes.length; j++) {
    tmp_arr.push( params.matrix[inst_row][j].value );
  }
...
function zoom_and_highlight_found_gene(search_gene){
  ...
  inst_gene_index = _.indexOf( params.all_genes, search_gene );  

  // get y position 
  inst_y_pos = params.y_scale(inst_gene_index)  ;
